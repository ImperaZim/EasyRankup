...
class JSON implements Provinder { 
 
 public function table() {
  return new Config(DataBase::getInstance()->getDataFolder() . "players.json");
 } 
 
 public function createTable() : void {
  (new Config(DataBase::getInstance()->getDataFolder() . "players.json", Config::JSON, ["players" => []]))->save(); 
 } 
 ...
 public function createProfile(Player $player) : void {
  $perfil = (new Config(DataBase::getInstance()->getDataFolder() . "players.json", Config::JSON, [
   "players" => [
    $player->getName() => [
 ...
 public function setValue(Player $player, String $param, $value) {
  if ($this->exist($player)) {
   $this->table()->setNested("players.{$player->getName()}.$param", $value);
  }
 } 
